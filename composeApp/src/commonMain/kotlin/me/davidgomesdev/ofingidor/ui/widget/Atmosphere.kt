...
@Composable
fun Atmosphere(modifier: Modifier = Modifier, starCount: Int = 120) {
    val stars = remember(starCount) {
        ...
        List(starCount) {
            ...
            Star(
                x = random.nextFloat(),
                y = random.nextFloat(),
                radius = if (kind < 0.1f) 1.2f else if (kind < 0.5f) 0.55f else 0.8f,
...
@Composable
fun BrandSigil(size: Dp = 38.dp, color: Color = purpleColor) {
    Canvas(Modifier.size(size)) {
        ...
        val star = Path().apply {
            val points = listOf(
                19f to 4f, 22.5f to 15.5f, 34f to 19f, 22.5f to 22.5f,
                19f to 34f, 15.5f to 22.5f, 4f to 19f, 15.5f to 15.5f,
            )
...
@Composable
fun SparkMark(size: Dp = 10.dp, color: Color = purpleColor) {
    Canvas(Modifier.size(size)) {
